	...
	if (strcmp(choice, "4") == 0) { //load
		...
		while (temp != NULL)
		{
			...
			if (temp->next == NULL)
			{
				...
				if (temp->song.timesPlayed < temp->prev->song.timesPlayed)
				{
					swap = temp->song;
					temp->song = temp->prev->song;
					temp->prev->song = swap;

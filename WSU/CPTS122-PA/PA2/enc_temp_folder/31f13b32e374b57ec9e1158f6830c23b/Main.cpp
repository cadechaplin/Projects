...
void sort() {
	...
	{
		Node* swap = temp->next;
		if (temp != NULL && swap != NULL) {
			if (strcmp(temp->song.name, swap->song.name) < 0)
			{


				if (temp->prev != NULL && swap->next != NULL) {//good, doesnt work for first or last item in list
					swap->next->prev = temp;
					swap->prev = temp->prev;
					temp->prev = swap;
					swap->prev->next = swap;
				...
				if (temp->prev == NULL && swap->next != NULL) {
					swap->next->prev = temp;
					swap->prev = temp->prev;
					temp->prev = swap;
